...
var matchedRoutePathContextKey struct{}

// MatchedRoutePathFromContext retrieves the path of the matched route from the request context.
func MatchedRoutePathFromContext(ctx context.Context) string {
	if v := ctx.Value(matchedRoutePathContextKey); v != nil {
		return v.(string)
	}
	return ""
}
...
func (r *Router) saveMatchedRoutePath(path string, handle Handle) Handle {
	return func(w http.ResponseWriter, req *http.Request, ps Params) {
		// Add the matched route path to the request context, so it can be used by native handlers
		req = req.WithContext(context.WithValue(req.Context(), matchedRoutePathContextKey, path))
		if ps == nil {
			psp := r.getParams()
			ps = (*psp)[0:1]
